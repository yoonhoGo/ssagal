...
0.1.0(소리 재생·연타 중첩·자동 반복·불타는 효과) 이후, 마우스 없이도 쓰고 취향대로 꾸밀 수 있도록 세 가지 기능이 추가됐습니다.

- ⌨️ **키보드 단축키** — `Space`/`Enter`로 실행, `3`/`5`/`0`으로 자동 반복, `R`로 계속, `Esc`/`S`로 중지. 마우스 없이 전부 제어됩니다.
- 🧭 **메뉴바(트레이) 아이콘** — macOS 메뉴바에 상주하며 **보이기/숨기기** 토글과 **종료** 메뉴를 제공합니다. 프레임 없는 창을 숨겼다가 다시 부를 수 있습니다.
- ⚙️ **설정 패널** — 우상단 톱니바퀴로 **이미지·사운드·반복 딜레이·불타는 효과 테마**를 바꾸고 `localStorage`에 저장합니다.

자세한 사용법은 아래 [기능](#기능) 표와 설명을 참고하세요.
...
- ♾️ **계속** — 중지 전까지 무한 재생
- ✨ **글자 효과** — `x10` 이상 자동 반복·계속 모드이거나 10회 이상 연타하면 선택한 효과 재생(더 많이 누를수록 강해짐)
...
src/
  ...
  styles.css     말풍선 디자인, 설정 패널 (번들 Jua 폰트)
  effects.css    클릭 팝, 불꽃/무지개/공주 효과
